...
namespace DL.FastCsvParser
{
    ...
    public class CsvParseException : Exception
    {
        ...
        public CsvParseException(string message, int position) : base(message)
        {
            Position = position;
        }

        public CsvParseException(string message, int position, Exception innerException) : base(message, innerException)
        {
            Position = position;
        }
    }
    ...
    public class Csv : List<List<string>>
    {
        ...
        /// Throws CsvParseException if the given string is an invalid CSV.
        /// </summary>
        public static Csv Parse(string csvContents)
        {
            if (csvContents == null)
            {
                return CsvParser.Parse(null);
            }
            return CsvParser.Parse(csvContents.Trim());
        }
        ...
        private static class CsvParser
        {
            ...
            public static Csv Parse(string csvContents)
            {
                ...
                for (int i = 0; i < totalNumChars; i++)
                {
                    ...
                    if (state == ParsingState.InField)
                    {
                        if (c == SEPARATOR) {
                            ...
                            throw new CsvParseException($"在位置 {i} 处，非转义字段中不能包含引号字符。", i);
                        }
                    }
                    ...
                    else if (state == ParsingState.AfterEscapedField)
                    {
                        if (c == SEPARATOR) {
                            ...
                            throw new CsvParseException($"在位置 {i} 处，转义字段关闭引号后不能有多余字符 '{c}'。", i);
                        }
                    }
                }
                ...
                if (state == ParsingState.InEscapedField) {
                    throw new CsvParseException($"在位置 {totalNumChars} 处，转义字段缺少关闭引号。", totalNumChars);
                }
                ...
                if (csv.Count > 0)
                {
                    ...
                    for (int i = 1, nRows = csv.Count; i < nRows; i++)
                    {
                        if (csv[i].Count != nFieldsInRow) {
                            throw new CsvParseException($"第 {i + 1} 行包含 {csv[i].Count} 个字段，但第一行包含 {nFieldsInRow} 个字段。所有行的字段数量必须相同。", totalNumChars);
                        }
                    }
                }
