...
public class NPC0NS2BulletRing : MonoBehaviour
{
    ...
    public void Refresh() {
        ...
        for (int i = 0; i < bulletNum; i++) {
            ...
            isAllNull = false;
           // if (isClockwise) {
                bullets[i].deg += 0.5f + Mathf.Sin(timer * Mathf.Deg2Rad + 360f / bulletNum /3f * i) * 0.1f;
            //}
           // else {
            //    bullets[i].deg -= 0.5f + Mathf.Sin(timer * 2f * Mathf.Deg2Rad + 360f / bulletNum /3f * i) * 0.1f;
            //}
            
            bullets[i].rad += (0.1f - timer / 500f) > 0.01f ? (0.1f - timer / 500f) : 0.01f;// * Mathf.Cos(timer * Mathf.Deg2Rad);
            
            bullets[i].bullet.transform.position = transform.position + bullets[i].rad * bullets[i].deg.Deg2Dir3();
        }
